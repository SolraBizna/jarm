...
public class SimFilesystem extends SimComponent {
	...
	private static enum Whence { SET, CUR, END }
	...
	private static class FileHandle {
		...
		public FileHandle(File path, boolean readable, boolean writable, boolean seekToEnd) throws FileNotFoundException {
			...
			if(seekToEnd) seek(Whence.END, 0);
		}
		public long seek(Whence whence, long offset) {
			try {
				...
				switch(whence) {
				default:
				case SET: base = 0; break;
				case CUR: base = raf.getFilePointer(); break;
				case END: base = max; break;
				}
	...
	private static class HandleValue extends AbstractValue {
		...
		static int getHandle(String me, Object o) {
			if(!(o instanceof HandleValue)) return MAX_HANDLES;
			HandleValue h = (HandleValue)o;
			if(h.owner.equals(me)) return h.handle;
			else return MAX_HANDLES;
		}
	};
	...
	public SimFilesystem(File basePath, boolean writable) {
		 ...
		 */
		this.address = String.format(filesystemAddressFormatString,
				basePath.getAbsolutePath().hashCode());
	...
	private File processOCPath(String path) {
		...
		for(String component : components) {
			if(component.equals(".")) {} // do nothing
	...
	Object[] open(Context ctx, Object[] args) {
		...
		if(!openForRead && !openForWrite) return new Object[]{null, "mode must begin with one of r, w, or a"};
		openForRead = openForRead || mode.contains("+");
		openForWrite = openForWrite || mode.contains("+");
		if(openForWrite && !writable) return new Object[]{null, "filesystem is read-only"};
		...
		if(filePath.exists() && filePath.isDirectory()) return new Object[]{null, "is a directory"};
		if(truncate) {
			try {
				if(filePath.exists() && !filePath.delete()) return new Object[]{null, "could not truncate file"};
				if(!filePath.createNewFile()) return new Object[]{null, "could not create file"};
			}
			catch(IOException e) {
	...
	public Object[] seek(Context ctx, Object[] args) {
		...
		if(handle >= MAX_HANDLES || handle < 0 || handles[handle] == null) return new Object[]{null, "no such handle"};
		String whenceStr = SimComponent.toString(args[1]);
		Whence whence;
		if(whenceStr.equals("set")) whence = Whence.SET;
		else if(whenceStr.equals("cur")) whence = Whence.CUR;
		else if(whenceStr.equals("end")) whence = Whence.END;
		else return new Object[]{null, "unknown whence value"};
	...
	public Object[] read(Context ctx, Object[] args) {
		...
		int readLen;
		if(Double.isInfinite(readLenN.doubleValue()) || readLenN.doubleValue() > MAX_READ_LENGTH) readLen = MAX_READ_LENGTH;
		else readLen = readLenN.intValue();
		if(readLen <= 0) return new Object[]{null, "read length out of range"};
		int handle = HandleValue.getHandle(address, args[0]);
		if(handle >= MAX_HANDLES || handle < 0 || handles[handle] == null) return new Object[]{null, "no such handle"};
		if(!handles[handle].readable) return new Object[]{null, "file not opened for reading"};
		byte[] buf = new byte[readLen];
		int red = handles[handle].read(buf);
		if(red < 0) return new Object[]{null, "IO error"};
		else if(red == 0) return new Object[]{null};
		else if(red < buf.length) return new Object[]{Arrays.copyOf(buf, red)};
		else return new Object[]{buf};
	}
	
	@Callback
	public Object[] write(Context ctx, Object[] args) {
		if(!writable) return new Object[]{null, "filesystem is read-only"};
		int handle = HandleValue.getHandle(address, args[0]);
		if(handle >= MAX_HANDLES || handle < 0 || handles[handle] == null) return new Object[]{null, "no such handle"};
		if(!handles[handle].writable) return new Object[]{null, "file not opened for writing"};
		byte[] buf;
